...
app = Blueprint('domain_a', __name__, url_prefix='/domain/<string:domain>/a',
                template_folder='templates/domain_a')
...
def record_list(domain):
    ...
    if not records:
        return redirect(url_for('domain.index', domain=domain.domain))
...
def record_delete(domain, record_id):
    ...
    return render_template('del.html', domain=domain, record=record)
